...
export default function AutoScheduleBestTimes() {
  ...
  async function handleSchedule(e) {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data?.error || "Unable to schedule best times.");
        return;
      }
      setResult(data);
    } catch {
      setError("Network error while scheduling best times.");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div style={{ marginTop: "20px" }}>
      ...
      <form onSubmit={handleSchedule}>
        ...
        <button type="submit" disabled={loading}>{loading ? "Scheduling..." : "Auto‑Schedule Best Times"}</button>
      </form>
