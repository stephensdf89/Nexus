...
export default function PinterestIntegration() {
  ...
  const [metrics, setMetrics] = useState<PinterestMetrics | null>(null);
  ...
  useEffect(() => {
    ...
    const checkStatus = async () => {
      try {
        setLoading(true);
        const res = await fetch("/api/integrations/pinterest/status", {
          headers: {
            "x-user-id": user.id || "",
        ...
        if (data.connected) {
          const metricsRes = await fetch(
            "/api/integrations/pinterest/analytics",
            {
              headers: {
                "x-user-id": user.id || "",
                "x-user-email": user.email || "",
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/integrations/pinterest/auth", {
        method: "POST",
  ...
  const handleDisconnect = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/integrations/pinterest/disconnect", {
        method: "POST",
        headers: {
          "x-user-id": user.id || "",
          "x-user-email": user.email || "",
  ...
  const handlePullAnalytics = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/integrations/pinterest/analytics", {
        headers: {
          "x-user-id": user.id || "",
          "x-user-email": user.email || "",
        },
      ...
      console.error("Error pulling Pinterest analytics:", err);
  ...
  if (loading) {
    return (
      <div className="flex flex-col justify-between rounded border border-cyan-400/40 bg-slate-900/50 p-6 h-full">
        <div>
          <div className="mb-4 h-12 w-12 rounded-full bg-[#E60023]"></div>
          <h2 className="text-xl font-semibold mb-3">Pinterest</h2>
      ...
      <div>
        ...
        {status.connected && metrics && (
          <div className="bg-slate-950/50 border border-cyan-400/20 rounded-lg p-3 mb-4 space-y-2">
            ...
            <div className="text-sm">
              <p className="text-gray-400">Pins</p>
              <p className="text-cyan-300 font-semibold">
                {metrics.videos}
              </p>
            </div>
