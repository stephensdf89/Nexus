...
export default function TikTokIntegration() {
  ...
  useEffect(() => {
    ...
    const checkStatus = async () => {
      try {
        setLoading(true);
        const res = await fetch("/api/integrations/tiktok/status", {
          headers: {
            "x-user-id": user.id || "",
        ...
        if (data.connected) {
          const metricsRes = await fetch(
            "/api/integrations/tiktok/analytics",
            {
              headers: {
                "x-user-id": user.id || "",
                "x-user-email": user.email || "",
        ...
        console.error("Error checking TikTok status:", err);
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/integrations/tiktok/auth", {
        method: "POST",
  ...
  const handlePullAnalytics = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/integrations/tiktok/analytics", {
        headers: {
          "x-user-id": user.id || "",
          "x-user-email": user.email || "",
        },
      ...
      console.error("Error pulling TikTok analytics:", err);
  ...
  if (loading) {
    return (
      <div className="flex flex-col justify-between rounded border border-cyan-400/40 bg-slate-900/50 p-6 h-full">
        <div>
          <div className="mb-4 h-12 w-12 rounded-full bg-[#3A7BFF]"></div>
          <h2 className="text-xl font-semibold mb-3">TikTok</h2>
