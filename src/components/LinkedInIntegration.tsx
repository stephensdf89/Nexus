...
export default function LinkedInIntegration() {
      ...
      try {
        ...
        if (data.connected) {
          const metricsRes = await fetch(
            "/api/integrations/linkedin/analytics",
            {
              headers: {
                "x-user-id": user.id || "",
                "x-user-email": user.email || "",
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/integrations/linkedin/auth", {
        method: "POST",
  ...
  const handlePullAnalytics = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/integrations/linkedin/analytics", {
        headers: {
          "x-user-id": user.id || "",
          "x-user-email": user.email || "",
        },
      ...
      console.error("Error pulling LinkedIn analytics:", err);
  ...
  if (loading) {
    return (
      <div className="flex flex-col justify-between rounded border border-cyan-400/40 bg-slate-900/50 p-6 h-full">
        <div>
          <div className="mb-4 h-12 w-12 rounded-full bg-[#0A66C2]"></div>
